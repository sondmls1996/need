...
public class DBHandle extends SQLiteOpenHelper {
    ...
    public static final String TABLE_LOCAL = "mylocal";
    ...
    public void addLocal(LocalConstructor listl) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(LAT, listl.getLat());
        values.put(LO, listl.getLo());
    ...
    public List<LocalConstructor> getAllLocal() {
        List<LocalConstructor> contactList2 = new ArrayList<LocalConstructor>();
        // Select All Query
        String selectQuery1 = "SELECT  * FROM " + TABLE_LOCAL;
        ...
        if (cursor1.moveToFirst()) {
            do {
                LocalConstructor contact2 = new LocalConstructor();
                contact2.setLat(cursor1.getDouble(0));
                contact2.setLo(cursor1.getDouble(1));
                // Adding contact to list
                contactList2.add(contact2);
            } while (cursor1.moveToNext());
        }
